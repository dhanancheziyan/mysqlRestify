...
var PATH = '/users'
server.get({path : PATH , version : '0.0.1'} , findActiveUsers);
server.get({path : PATH +'/deleted' , version : '0.0.1'} , findDeletedUsers);
server.get({path : PATH +'/updated/:timestamp' , version : '0.0.1'} , findUpdatedUsers);
server.get({path : PATH +'/:tid' , version : '0.0.1'} , findUser);
server.post({path : PATH , version: '0.0.1'} ,addUser);
server.del({path : PATH +'/:tid' , version: '0.0.1'} ,deleteUser);
server.put({path : PATH +'/:tid' , version: '0.0.1'} ,updateUser);
...
function addUser(req , res , next){
    ...
    user.last_name =req.params.last_name;j
...
server.get({path : VEH_PATH , version : '0.0.1'} , findActiveVehicles);
server.get({path : VEH_PATH +'/deleted' , version : '0.0.1'} , findDeletedVehicles);
server.get({path : VEH_PATH +'/updated/:timestamp' , version : '0.0.1'} , findUpdatedVehicles);
server.get({path : VEH_PATH +'/:vehicle_number' , version : '0.0.1'} , findVehicle);
server.post({path : VEH_PATH , version: '0.0.1'} ,addVehicle);
server.del({path : VEH_PATH +'/:vehicle_number' , version: '0.0.1'} ,deleteVehicle);
server.put({path : VEH_PATH +'/:vehicle_number' , version: '0.0.1'} ,updateVehicle);
...
function addVehicle(req , res , next){
	...
	connection.query("INSERT INTO vehicles set ?",[vehicle], function(err, rows) {
		...
		res.json(200, {vehicle_number: req.params.vehicle_number});
	});
	return next();
}
...
server.get({path : SPECIAL_INSTRUCTIONS_PATH , version : '0.0.1'} , findActiveInstructions);
server.get({path : SPECIAL_INSTRUCTIONS_PATH +'/deleted' , version : '0.0.1'} , findDeletedInstructions);
server.get({path : SPECIAL_INSTRUCTIONS_PATH +'/updated/:timestamp' , version : '0.0.1'} , findUpdatedInstructions);
server.get({path : SPECIAL_INSTRUCTIONS_PATH +'/:vehicle_number' , version : '0.0.1'} , findInstruction);
server.post({path : SPECIAL_INSTRUCTIONS_PATH , version: '0.0.1'} ,addInstruction);
server.del({path : SPECIAL_INSTRUCTIONS_PATH +'/:vehicle_number' , version: '0.0.1'} ,deleteInstruction);
server.put({path : SPECIAL_INSTRUCTIONS_PATH +'/:vehicle_number' , version: '0.0.1'} ,updateInstruction);
...
function findInstructions(req, res, deleted, next){
	...
	connection.query('SELECT * FROM special_instructions WHERE is_active = ?', [deleted], function(err, rows, fields) {
	  if (err) throw err;
		res.json(200, { special_instructions: rows});
	});
	return next();
}
